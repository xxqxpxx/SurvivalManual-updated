...
fun MainScreen(
    ...
    Box(
        ...
        Column(
            ...
            when (state) {
                is MainScreenState.Loading -> CircularProgressIndicator(modifier = Modifier.align(Alignment.CenterHorizontally))
                is MainScreenState.Success -> {
                    val content = (state as MainScreenState.Success).content
                    Text(text = "Survival Guide Loaded: ${content.title}", modifier = Modifier.padding(8.dp))
                }
                ...
                is MainScreenState.SearchResults -> {
                    ...
                    LazyColumn(modifier = Modifier.fillMaxSize()) {
                        items(results) { result -> // Assuming SearchResult has a 'snippet' field
                            ...
                            Text(
                                text = highlightTextAsAnnotatedString(text = result.snippet, query = searchQuery),
                                modifier = Modifier.padding(horizontal = 8.dp, bottom = 8.dp))
                            Text(text = result.title, modifier = Modifier.padding(8.dp))
                     }
                    }
                }
                MainScreenState.Initial -> Text(text = "Initializing...", modifier = Modifier.padding(8.dp))
            }
        }
    }
}
